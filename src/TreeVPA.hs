...
-- | derive is equivalent to the original derivative of a Tree in Tree.hs.
-- | It handles the matching of the NodeExpr itself and
-- | splits up the other operator logic between deriveCall and deriveReturn.
derive :: Expr -> Tree -> Expr
derive x (Tree a children) =
    let nodeExprs = deriveCall x in
    let ms = map (\(NodeExpr b thn) -> if a == b then match thn children else False) nodeExprs in
    let (dx, []) = deriveReturn x ms in
    dx
...
vpaCall xs a =
    let nodeExprs = concatMap deriveCall xs in
    let xs' = map (\(NodeExpr b thn) -> if a == b then thn else EmptySet) nodeExprs in
    (xs, xs')

-- | vpaReturn' is a helper function for vpaReturn.
-- | vpaReturn' calls deriveReturn for each expression, eating the concatenated list of matches from the head as it goes.
-- | vpaReturn' terminates when it has executed deriveReturn for each expression and the list of matches is empty.
-- | vpaReturn' :: exprs -> matches -> returned_exprs
vpaReturn' :: [Expr] -> [Bool] -> [Expr]
vpaReturn' [] [] = []
vpaReturn' (x:xs) ms =
  let (x', ms') = deriveReturn x ms in x' : vpaReturn' xs ms'
